...
<template>
    <Dialog title="Edit Tool" :open="open" @update:open="updateOpenState" boxClass="w-full max-w-xl">
        <form class="flex flex-col gap-4 flex-grow w-full max-w-2xl max-h-full overflow-auto" @submit.prevent="">
            <div class="flex flex-col md:flex-row items-start gap-4">
                <ImageUpload class="flex-shrink-0" :image="image" @update:image="image = $event" :blob="data.image" :loading="creating" />
            ...
            <button
                class="flex justify-center w-max p-3 px-6 rounded-lg bg-blue-400 text-black1 hover:bg-blue-500 hover:text-white group"
                type="button"
                @click="add()"
            >
                <div class="flex items-center gap-2" v-if="!creating">
                    <Icon class="icon w-5 h-5 bg-black2 group-hover:bg-white" name="edit.svg" folder="icons" size="20px" />Edit
                </div>
...
const add = async () => {
    ...
    await axios
        .put(`/api/v1/admin/tech-and-tools/${props.data._id}`, data)
        .then((response) => {
            toast.info("Tool has been updated!", { timeout: 2000 });
